...
METRICS_FILE="$PROJECT_ROOT/psi/pulse/metrics.jsonl"
METRICS_SUMMARY="$PROJECT_ROOT/psi/pulse/metrics-summary.json"
...
" 2>/dev/null || echo 0)
  ...
  local events_file="$PROJECT_ROOT/psi/pulse/events.jsonl"
...
" 2>/dev/null || echo 0)
  ...
  local pid_dir="$PROJECT_ROOT/psi/pulse/daemon-logs"
  if [[ -d "$pid_dir" ]]; then
