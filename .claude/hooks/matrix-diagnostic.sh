...
if [[ -f "$PROJECT_ROOT/scripts/voice/voice.sh" ]]; then
    echo "✅ Found"
else
    echo "❌ MISSING (scripts/voice/voice.sh)"
    errors=$((errors+1))
fi

echo -n "  voice_server.py (daemon)... "
if [[ -f "$PROJECT_ROOT/scripts/voice/voice_server.py" ]]; then
    echo "✅ Found"
else
    echo "❌ MISSING (scripts/voice/voice_server.py)"
    errors=$((errors+1))
fi
...
if [[ -n "$SERVER_PID" ]]; then
    ...
    echo "❌ NOT RUNNING"
    echo "     Start with: python3 scripts/voice/voice_server.py &"
    errors=$((errors+1))
fi
...
if [[ -n "$SERVER_PID" ]] && [[ -f "$PROJECT_ROOT/scripts/voice/voice.sh" ]]; then
    echo -n "  Sending test message to Oracle... "
    cd "$PROJECT_ROOT"
    bash scripts/voice/voice.sh "Diagnostic complete." "Oracle" 2>/dev/null
...
if [[ "$errors" -eq 0 ]] && [[ "$warnings" -eq 0 ]]; then
    ...
    echo ""
    echo "Common fixes:"
    echo "  - Start server: python3 scripts/voice/voice_server.py &"
